...
func (s *deltaHistogram[N]) measure(
	...
	h := s.hotColdValMap[hotIdx].LoadOrStoreAttr(attribute.NewDistinct(fltrAttrs...), func(overflow bool) any {
		attr := overflowSet
		if !overflow {
...
func (s *cumulativeHistogram[N]) measure(
	...
	h := s.values.LoadOrStoreAttr(attribute.NewDistinct(fltrAttrs...), func(overflow bool) any {
		attr := overflowSet
		if !overflow {
			attr = attribute.NewSet(fltrAttrs...)
		}
