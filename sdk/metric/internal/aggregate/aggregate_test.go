	...
	return func(t *testing.T) {
		...
		run := func(b Builder[N], wantF attribute.Set, wantD []attribute.KeyValue) func(*testing.T) {
			return func(t *testing.T) {
				...
				meas := b.filter(func(_ context.Context, v N, attrs []attribute.KeyValue, d []attribute.KeyValue) {
					assert.Equal(t, value, v, "measured incorrect value")
					assert.Equal(t, wantF, attribute.NewSet(attrs...), "measured incorrect filtered attributes")
					assert.ElementsMatch(t, wantD, d, "measured incorrect dropped attributes")
				})
				meas(t.Context(), value, attr.ToSlice())
			}
		}
	...
	return func(t *testing.T) {
		...
		for i, step := range steps {
			for _, args := range step.input {
				meas(args.ctx, args.value, args.attr.ToSlice())
			}
